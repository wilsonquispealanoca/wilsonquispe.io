...
function Navbar() {
  ...
  const Day = () => <svg className="dark:fill-primary-dark" width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const Moon = () => <svg className="fill-primary-text" width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <>
    <nav className={`${show ? "bg-white dark:bg-card-dark" : "bg-none"} flex items-center w-full justify-between h-[4rem] lg:h-28 fixed left-0 right-0 top-0 mx-auto z-40 px-6 md:px-32 lg:px-56 xl:px-80 text-xl font-semibold`}>
        <figure onClick={() => setMenuOpen(!menuOpen)} className={`flex items-center justify-center w-10 h-10 cursor-pointer lg:hidden`}>
    ...
    {menuOpen && <NavMobile open={menuOpen}/>}
    </>
  );
}
